 ...
 * @return string
 */
function theme_dfsnz_process_css($css, $theme) {
    ...
    $css = theme_dfsnz_set_customcss($css, $customcss);
 ...
 * @return string
 */
function theme_dfsnz_set_customcss($css, $customcss) {
 ...
 * @return bool
 */
function theme_dfsnz_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = array()) {
    if ($context->contextlevel == CONTEXT_SYSTEM && ($filearea === 'logo' || $filearea === 'favicon' || $filearea === 'backgroundimage')) {
        $theme = theme_config::load('dfsnz');
